...
class RecogNetWrapper(nn.Module):
    ...
    def forward(self, image, M):
        import cv2
        img=image[0].permute(1, 2, 0).detach().cpu().numpy()*255
        img_new = img[:, :, ::-1]
        cv2.imwrite("./models/1.jpg", img_new)

        image=resize_n_crop(image, M, self.input_size)

        img = image[0].permute(1, 2, 0).detach().cpu().numpy() * 255
        img_new = img[:, :, ::-1]
        cv2.imwrite("./models/2.jpg", img_new)

        image = self.preprocess(image)

        img = image[0].permute(1, 2, 0).detach().cpu().numpy() * 255
        img_new = img[:, :, ::-1]
        cv2.imwrite("./models/3.jpg",img_new)
