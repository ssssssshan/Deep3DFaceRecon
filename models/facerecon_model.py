...
class FaceReconModel(BaseModel):
    ...
    def forward(self):
        ...
        # for i in range(self.pred_lm.shape[0]):
        #     output_vis_numpy_raw_gray = cv2.cvtColor(util.tensor2im(output_vis_numpy_raw[i]), cv2.COLOR_BGR2GRAY)
        #     rects = detector(output_vis_numpy_raw_gray, 1)
        #     cv2.imwrite('output.jpg', output_vis_numpy_raw_gray)
        #     landmark_ = np.array([[p.x, p.y] for p in predictor(output_vis_numpy_raw_gray, rects[0]).parts()])
        #     landmark_[:, 1] = output_vis_numpy_raw_gray.shape[0] - 1 - landmark_[:, 1]
        #     self.pred_lm[i]=torch.from_numpy(landmark_)
        #     # draw_landmarks(output_vis_numpy_raw[i], landmark_, 'output_lm.jpg')
