...
def detect_68p(img_path,sess,input_op,output_op):
    ...
    vis_dlib_path = os.path.join(img_path, 'vis_dlib')
    remove_path = os.path.join(img_path, 'remove')
    save_path = os.path.join(img_path, 'landmarks')
    save_dlib_path = os.path.join(img_path, 'landmarks_dlib')
    if not os.path.isdir(vis_path):
        os.makedirs(vis_path)
    if not os.path.isdir(vis_dlib_path):
        os.makedirs(vis_dlib_path)
    if not os.path.isdir(remove_path):
        os.makedirs(remove_path)
    if not os.path.isdir(save_path):
        os.makedirs(save_path)
    if not os.path.isdir(save_dlib_path):
        os.makedirs(save_dlib_path)
    ...
    for i in range(0, len(names)):
        ...
        input_img_,scale, bbox = align_for_lm(img, five_points) # align for 68 landmark detection
        ...
        input_img = np.reshape(
            input_img_, [1, 224, 224, 3]).astype(np.float32)
        landmark = sess.run(
        ...
        input_img_gray = cv2.cvtColor(input_img_, cv2.COLOR_BGR2GRAY)
        rects = detector(input_img_gray, 1)
        if len(rects) == 1:
            landmark_= np.array([[p.x, p.y] for p in predictor(input_img_gray, rects[0]).parts()])
            landmark_ = landmark_ / scale
            landmark_[:, 0] = landmark_[:, 0] + bbox[0]
            landmark_[:, 1] = landmark_[:, 1] + bbox[1]
            landmark_[:, 1] = img.shape[0] - 1 - landmark_[:, 1]
        else:
            move(full_image_name, os.path.join(remove_path, name))
            continue
        ...
        if i % 100 == 0:
            ...
            draw_landmarks(img, landmark_, os.path.join(vis_dlib_path, name))
        save_label(landmark, os.path.join(save_path, txt_name))
        save_label(landmark_, os.path.join(save_dlib_path, txt_name))
